...
// 로그 디렉토리 생성 (에러 핸들링 포함) - 테스트 환경에서는 생략
...
// Pino 인스턴스 - 테스트 환경에서는 transport 사용하지 않음
const getLoggerConfig = () => {
  if (isTest) {
    // 테스트 환경: transport 없이 간단한 destination 사용
    return {
  ...
  // production/development 환경: transport 사용
    ...
    ? [
        // prod: 일자/용량 기반 롤링 + 30일 보관 (전체 로그)
        {
          ...
          options: {
            ...
            extension: '.log',
            mkdir: true,
            symlink: false, // symlink 비활성화로 에러 방지
...
export const logger = pino(
  ...
  loggerConfig.transport || loggerConfig.destination,
);
